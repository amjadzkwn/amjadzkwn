# 👋 Hello, I'm Amjad

I'm a final-year **Data Science** student at **Universiti Malaysia Sabah (UMS)**, passionate about leveraging data to solve real-world problems. My primary focus is on **Machine Learning**, **Generative AI**, and **Natural Language Processing**, particularly in the financial domain.

## 🎓 Current Focus
- **Final Year Thesis:** Building a hybrid model for stock market prediction using Machine Learning and NLP techniques.
- Exploring how alternative data like financial news can improve financial forecasting models.

## 🔧 Technical Toolkit
**Languages & Frameworks:** Python, SQL, R  
**Data Science & ML:** Pandas, NumPy, Scikit-learn, TensorFlow/Keras, Statsmodels  
**Data Visualization:** Matplotlib, Seaborn, Plotly  
**Tools & Platforms:** Git, GitHub, Jupyter Notebook, VS Code, Google Colab

## 📈 Projects & Interests
- Predictive Analytics & Time Series Forecasting
- Sentiment Analysis & Text Mining
- Data Engineering & Pipeline Building
- Model Deployment & MLOps Basics

## 📫 Let's Connect
- **Email:** amjadzakwan5@gmail.com
- **LinkedIn:** [[Your LinkedIn Profile Link](https://www.linkedin.com/in/amjad-zakwan/)]

> *"Turning raw data into actionable insights and intelligent predictions."*

<!--
**amjadzkwn/amjadzkwn** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
